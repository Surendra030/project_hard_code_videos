...
def hardcode_all_videos(video, index=1):
    ...
    if len(audio_files) > 0 and len(sub_files) > 0:
        ...
        if len(srt_sub_files) > 0:
            s_file = srt_sub_files[-1]
        ...
        print("Video file can't be hardcoded..")
        output_file = f'{str(video).split(".")[0]}_hardcoded.mp4'
